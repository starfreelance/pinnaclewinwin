package com.pinnacle.winwin.ui.motorgame;
...
public class MotorGameScreenActivity extends ASOnlineBaseActivity implements
    ...
    private static final String TAG = MotorGameScreenActivity.class.getSimpleName();
    ...
    private ArrayList<SingleGameNumberModel> motorGameNumberList;
    private ArrayList<PointsModel> pointsList;

    private ArrayList<AmountDetailsResponse> amountDetails;
    private ArrayList<MotorCombDetailsResponse> motorCombDetails;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_motor_game_screen);
        ...
        handler = new Handler();
        motorGameNumberList = getMotorGameNumberList();
        if (motorGameNumberList != null && !motorGameNumberList.isEmpty()) {
        ...
        motorCombDetails = fetchMotorCombDetailsFromDb();
    ...
    private void initViews() {
        ...
        lblSingleGame.setText(getResources().getString(R.string.title_motor_game));
    ...
    private ArrayList<MotorCombDetailsResponse> fetchMotorCombDetailsFromDb() {
        return (ArrayList<MotorCombDetailsResponse>) ASOnlineApplication.appDatabase.motorCombDetailsDao().getMotorCombDetails();
    }
    ...
    private ArrayList<SingleGameNumberModel> getMotorGameNumberList() {

        ArrayList<SingleGameNumberModel> motorGameNumberList = new ArrayList<>();

        String[] motorGameNumberArray = getResources().getStringArray(R.array.game_number_array);

        for (int i = 0; i < motorGameNumberArray.length; i++) {
            SingleGameNumberModel motorGameNumberModel = new SingleGameNumberModel();
            motorGameNumberModel.setNumber(motorGameNumberArray[i]);
            motorGameNumberModel.setPointsValue(0);
            motorGameNumberModel.setSelected(false);

            motorGameNumberList.add(motorGameNumberModel);
        }
    ...
    private void loadGenericNumberAdapter() {
        if (genericNumberAdapter == null) {
            genericNumberAdapter = new GenericNumberAdapter(this, motorGameNumberList, gameId);
    ...
    private void clearAllBetData() {
        ...
        genericNumberAdapter.updateData(motorGameNumberList);
    }

    /*private void updateTotalPoints() {
        int totalPoints = 0;
        for (SingleGameNumberModel motorGameNumberModel : motorGameNumberList) {
            if (motorGameNumberModel.isSelected()) {
                totalPoints = totalPoints + motorGameNumberModel.getPointsValue();
            }
        }
    ...
    private int getSelectedCombinationCount() {
        int count = 0;
        if (getSelectedNumberList().size() > 3 && (checkboxSP.isChecked() ||
                checkboxDP.isChecked() || checkboxTP.isChecked())) {
            MotorCombDetailsResponse motorCombDetail = getSelectedMotorComboDetail();
            if (motorCombDetail != null) {
                if (checkboxSP.isChecked() &&
                        checkboxDP.isChecked() && checkboxTP.isChecked()) {
                    count = motorCombDetail.getSpComb() + motorCombDetail.getDpComb() + motorCombDetail.getTpComb();
                } else if ((checkboxSP.isChecked() && checkboxDP.isChecked() && !checkboxTP.isChecked())) {
                    count = motorCombDetail.getSpComb() + motorCombDetail.getDpComb();
                } else if (checkboxSP.isChecked() && !checkboxDP.isChecked() && checkboxTP.isChecked()) {
                    count = motorCombDetail.getSpComb() + motorCombDetail.getTpComb();
                } else if (!checkboxSP.isChecked() && checkboxDP.isChecked() && checkboxTP.isChecked()) {
                    count = motorCombDetail.getDpComb() + motorCombDetail.getTpComb();
                } else if (checkboxSP.isChecked() ||
                        checkboxDP.isChecked() || checkboxTP.isChecked()) {
                    if (checkboxSP.isChecked()) {
                        count = motorCombDetail.getSpComb();
                    } else if (checkboxDP.isChecked()) {
                        count = motorCombDetail.getDpComb();
                    } else if (checkboxTP.isChecked()) {
                        count = motorCombDetail.getTpComb();
                    }
                }
    ...
    private MotorCombDetailsResponse getSelectedMotorComboDetail() {
        MotorCombDetailsResponse mMotorCombDetail = null;
        for (MotorCombDetailsResponse motorCombDetail : motorCombDetails) {
            if (motorCombDetail.getPaanaCount() == getSelectedNumberList().size()) {
                mMotorCombDetail = motorCombDetail;
                break;
            }
        }

        return mMotorCombDetail;
    }

    private void computeMotorLogic() {
        if (getSelectedCombinationCount() > 0 && selectedPoints > 0) {
            totalPoints = selectedPoints * getSelectedCombinationCount();
            btnTotal.setText(String.valueOf(totalPoints));
            textViewMotorPoints.setText("");
            textViewMotorPoints.append(getResources().getString(R.string.lbl_selected_points));
    ...
    /*private int getNumberOfGameCount() {
        if (checkboxSP.isChecked() &&
                checkboxDP.isChecked() && checkboxTP.isChecked()) {
            return 3;
        } else if ((checkboxSP.isChecked() && checkboxDP.isChecked() && !checkboxTP.isChecked()) ||
                (checkboxSP.isChecked() && !checkboxDP.isChecked() && checkboxTP.isChecked()) ||
                (!checkboxSP.isChecked() && checkboxDP.isChecked() && checkboxTP.isChecked())) {
            return 2;
        } else if (checkboxSP.isChecked() ||
                checkboxDP.isChecked() || checkboxTP.isChecked()) {
    ...
    private ArrayList<SingleGameNumberModel> getSelectedNumberList() {
        ...
        if (motorGameNumberList != null && !motorGameNumberList.isEmpty()) {
            for (SingleGameNumberModel motorGameNumberModel : motorGameNumberList) {
                if (motorGameNumberModel.isSelected()) {
                    selectedGameNumberList.add(motorGameNumberModel);
                }
            }
        }
...
//        intent.putExtra(AppConstant.KEY_BAAZAAR_ID, baazaarId);
    ...
    private ArrayList<BetPaanaData> getBetPaanaDataList() {
        ...
        if (checkboxSP.isChecked()) {
            ...
            betPaanaData.setSelectedPaana(getSelectedPaana());
            betPaanaData.setAmountPerPaana(selectedPoints);
            /*betPaanaData.setTotalAmount(totalPoints);*/
            betPaanaData.setTotalAmount(selectedPoints * getSelectedMotorComboDetail().getSpComb());
            betPaanaData.setPaanaType(AppConstant.PAANA_TYPE_SP);

            betPaanaDataList.add(betPaanaData);
        }

        if (checkboxDP.isChecked()) {
            BetPaanaData betPaanaData = new BetPaanaData();
            betPaanaData.setSelectedPaana(getSelectedPaana());
            betPaanaData.setAmountPerPaana(selectedPoints);
            /*betPaanaData.setTotalAmount(totalPoints);*/
            betPaanaData.setTotalAmount(selectedPoints * getSelectedMotorComboDetail().getDpComb());
        ...
        if (checkboxTP.isChecked()) {
            ...
            betPaanaData.setAmountPerPaana(selectedPoints);
            /*betPaanaData.setTotalAmount(totalPoints);*/
            betPaanaData.setTotalAmount(selectedPoints * getSelectedMotorComboDetail().getTpComb());
    ...
    private String getSelectedPaana() {
        StringBuilder selectedPaana = new StringBuilder();
        if (motorGameNumberList != null && !motorGameNumberList.isEmpty()) {
            for (SingleGameNumberModel motorGameNumberModel : motorGameNumberList) {
                if (motorGameNumberModel.isSelected()) {
                    selectedPaana.append(motorGameNumberModel.getNumber());
                }
            }
        }

        return selectedPaana.toString();
    }
    ...
    public void onIndexSelectedListener(int index) {
        if (!motorGameNumberList.get(index).isSelected()) {
            motorGameNumberList.get(index).setSelected(true);
            genericNumberAdapter.updateData(motorGameNumberList);
            computeMotorLogic();
        } else {
            motorGameNumberList.get(index).setSelected(false);
            genericNumberAdapter.updateData(motorGameNumberList);
            computeMotorLogic();
        }
    }
    ...
    public void onPointsSelectListener(/*PointsModel pointsModel*/ AmountDetailsResponse amountDetail) {
        if (amountDetail != null) {
            if (getSelectedNumberList().size() > 0) {
                ...
                if (totalPoints <= AppConstant.MAX_MOTOR_GAME_BET_POINTS) {
                    selectedPoints = selectedPoints + amountDetail.getAmountValue();
                    computeMotorLogic();
                } else {
                    Utils.showCustomSnackBarMessageView(this, parentLayout, getResources().getString(R.string.maximum_motor_game_bet_error),
                            Snackbar.LENGTH_LONG, getResources().getString(R.string.btn_okay)).show();
                }
            }
        }
