...
public class CryptLib {
    ...
    public String encryption(String _plainText, String key, String iv) throws
        ...
        /*byte[] hexKey = new BigInteger(key, 16).toByteArray();*/
        SecretKey secretKey = new SecretKeySpec(hexStringToByteArray(key), "AES");


        /*byte[] hexIV = new BigInteger(iv, 16).toByteArray();*/

        IvParameterSpec ivSpec = new IvParameterSpec(hexStringToByteArray(iv)); // Create a new

        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5PADDING");

        cipher.init(Cipher.ENCRYPT_MODE, secretKey, ivSpec);// Initialize this cipher instance
        byte[] results = cipher.doFinal(_plainText.getBytes()); // Finish

        return bytesToHex(results);

    }

    public String decryption(String encryptedText, String key, String iv) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException {
        ...
        byte[] results = cipher.doFinal(hexStringToByteArray(encryptedText));

        return new String(results).replaceAll("(?:\\r\\n|\\n\\r|\\n|\\r)", "");
    }
